...
import { TrendingUp, TrendingDown, ArrowDown, ArrowUp, AlertTriangle, CheckCircle, Landmark, Wallet, BarChart3, MessageSquare, DollarSign, PieChart } from "lucide-react";
import { CLUB_SHORT } from "@/lib/constants";
import Avatar, { titleCase } from "@/components/Avatar";
import AdminNotifications from "@/components/AdminNotifications";

function MemberDashboard({ data, clubActivity = [], hideHeader = false }) {
...
function AdminDashboard({ members, portfolio, activity }) {
...
export default function DashboardClient({ user, memberData, adminData }) {
  if (user.role === "admin") {
    return (
      <>
        <AdminNotifications />
        ...
        <div className="mt-8 pt-8 border-t border-surface-3">
          ...
          <MemberDashboard data={memberData} clubActivity={adminData.activity} hideHeader />
        </div>
      </>
    );
  }

  return <MemberDashboard data={memberData} />;
}
