...
async function computeClubShares(cutoffDate) {
  ...
  for (const c of (allContribs || [])) {
    ...
    if (c.type === "withdrawal" || c.type === "expense") {
      memberShares[c.member_id].net -= c.amount;
    }
  }
...
// falling back to portfolio_snapshots when price_history has no data yet.
export async function getHistoricalPortfolioValue(asOf) {
  ...
  // If price_history has entries for this date, use them
  if (history?.length > 0) {
  ...
  // Fallback: use the nearest portfolio_snapshot at or before the requested date
  const asOfDate = typeof asOf === "string" ? asOf.split("T")[0] : new Date(asOf).toISOString().split("T")[0];
  ...
  return snap?.total_value ? parseFloat(snap.total_value) : null;
}
